...
def build_optimizers(model, hp, sb_idx):
    ...
    # Plain SGD for index_logits — NO momentum, NO fp32 master (minimal memory)
    # AdamW: 2×4=8 bytes/param × 1.78B = 14.2 GB — too much
    # SGD with momentum: 1×4=4 bytes/param × 1.78B = 7.1 GB — still too much
    # Plain SGD: 0 extra bytes — only uses the gradient (already allocated)
    opt_indices = torch.optim.SGD(plain_adamw_groups, lr=lrs.get("indices", 1e-3), momentum=0.0) if plain_adamw_groups else None
...
def build_student_super_block(sb_idx, lora_rank=16, lora_alpha=32, use_soft_indices=False):
    ...
    # Attach LoRA rank-16 on ALL palettized Linears (not just correction layer)
    total_lora = 0
    for layer_idx in range(sb_start, sb_end):
        layer = model.model.layers[layer_idx]
        n_lora = attach_lora_to_layer(layer, rank=lora_rank, alpha=lora_alpha,
                                       original_weights=None, layer_name=f"model.layers.{layer_idx}")
        total_lora += n_lora
    print(f"  Attached LoRA rank-{lora_rank} to {total_lora} Linears across {sb_end - sb_start} layers", flush=True)
...
def save_state(model, sb_idx, step, cos, loss, out_dir):
    ...
    print(f"  Saved {n_saved} trainable params (step={step}, cos={cos:.6f})", flush=True)


def load_state(model, resume_dir):
    """Load saved trainable params from resume_dir into model.

    Returns: (step, cos, loss) from _resume.json, or (0, -1, inf) if not found.
    """
    ...
    if n_missing > 0:
        print(f"  WARNING: {n_missing} trainable params not found in resume dir "
...
def train_super_block(sb_idx, max_steps, lora_rank=16, lora_alpha=32, seq_len=128, batch_size=8,
    ...
    for batch_ids in data_stream:
        ...
        # Logging — show per-group LRs
        if global_step % hp.get("log_every", 50) == 0:
            cos_val = 1.0 - comps["cos"]
            tps = (global_step - resume_step) / max(1e-6, time.time() - start_time)
            # Collect unique group→LR pairs across both optimizers
            group_lrs = {}
            if opt_muon:
            ...
            print(f"  step={global_step:5d} loss={comps['loss']:.6f} cos={cos_val:.6f} tps={tps:.1f} [{lrs_str}]", flush=True)
...
def main():
    ...
    ap.add_argument("--seq_len", type=int, default=128,
                    help="Tokens per sequence. Lower = faster step + more grad updates per token.")
    ap.add_argument("--batch_size", type=int, default=8,
                    help="Sequences per step. Higher keeps GPU busy at short seq_len.")
